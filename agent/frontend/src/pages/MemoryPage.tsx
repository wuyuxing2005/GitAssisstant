...
export function MemoryPage() {
  ...
  async function loadMemories() {
    try {
      ...
      setExpandedMemoryId((current) => current ?? response.items[0]?.id ?? null);
    } catch (error) {
  ...
  async function handleRebuild() {
    try {
      ...
      await loadMemories();
      setExpandedMemoryId(response.items[0]?.id ?? null);
  ...
  async function handleDelete(memory: LongTermMemoryRecord) {
    const confirmed = window.confirm(`确定删除记忆 "${memory.task_name}" 吗？`);
    if (!confirmed) {
      return;
    }
    try {
  ...
  async function handleClear() {
    const confirmed = window.confirm("确定清空全部长期记忆吗？此操作不会删除任务记录。");
    if (!confirmed) {
      return;
    }
    try {
    ...
    <section className="card memory-panel">
      <div className="section-header">
        ...
        <div className="action-row">
          ...
          <button className="ghost-button danger" type="button" onClick={() => void handleClear()} disabled={busy || rebuilding || memories.length === 0}>
            清空
          </button>
        </div>
      </div>
