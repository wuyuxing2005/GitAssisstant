...
from .cli_utils.git import run_git_capture
...
def create_github_pr(repo_path: str, branch: str, base_branch: str, title: str, body: str) -> str:
    token = os.getenv("GITHUB_TOKEN", "").strip()
    if not token:
        raise RuntimeError("缺少 GITHUB_TOKEN，无法创建 PR。")
    repo_info = parse_github_remote(repo_path)
    if repo_info is None:
        raise RuntimeError("当前 origin 不是可识别的 GitHub 仓库，无法创建 PR。")
