...
from ..repositories.session_state_repository import (
...
class SessionService:
    def __init__(self, orchestrator, workspace_root: str | Path | None = None,
                 sandbox_manager=None, store: SessionStateRepository | None = None):
        ...
        self.store = store or SessionStateRepository(default_session_state_db_path(self.workspace_root))
        # session_id -> Session
    ...
    def _restore_graph_state(self, session: Session) -> None:
        ...
        if existing_snapshot and existing_snapshot.values:
            return
        last_node = record.last_node or "__start__"
        try:
    ...
    def get_session_by_thread_id(self, thread_id: str) -> Session:
        """根据 LangGraph thread_id 获取已有会话。"""
        for session in self.sessions.values():
            if session.thread_id == thread_id:
                return session
        raise ValueError(f"会话不存在: thread_id={thread_id}")

    def switch_session_by_thread_id(self, thread_id: str) -> Session:
        """根据 thread_id 切换到已有会话，并恢复 graph state。"""
        session = self.get_session_by_thread_id(thread_id)
        self._switch_to(session)
        return session
