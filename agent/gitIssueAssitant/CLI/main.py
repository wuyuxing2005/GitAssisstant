...
from ..core.services.issue_assistant_service import IssueAssistantService
from prompt_toolkit import PromptSession
from prompt_toolkit.patch_stdout import patch_stdout

class CLI:
    def __init__(self, assistant: IssueAssistantService):
    ...
    async def _drain_injections(self, thread_id: str) -> int:
        ...
        while True:
            ...
            if text.startswith("!replan"):
                ...
                self.assistant.inject_message(thread_id, payload, replan=True)
                print(f"📨 已注入并标记重规划: {payload}")
            else:
                self.assistant.inject_message(thread_id, text)
    ...
    async def handle_command(self, cmd: str):
        ...
        try:
            ...
            if command == "/repo" or command == "/new":
                ...
                session, duplicates = self.assistant.create_session(repo_ref, target_dir=target_dir, force=force)
            ...
            # ===== 切换会话 =====
                ...
                if len(parts) < 2:
                    ...
                    return
                session = self.assistant.switch_session(parts[1])
            ...
            # ===== 列出会话 =====
                ...
                if len(parts) > 2:
                    ...
                    return
                if len(parts) == 2:
                    session = self.assistant.get_session(parts[1])
                    state = self.assistant.get_session_state(parts[1])
                    marker = "是" if session.session_id == self.assistant.current_session_id else "否"
                    ...
                    if not sessions:
                        ...
                        current_id = self.assistant.current_session_id
                        for s in sessions:
                ...
                if len(parts) < 2:
                    ...
                    return
                issue = " ".join(parts[1:])
                self.assistant.set_issue(issue)
                session = self.assistant.current_session()
                print(f"📝 会话 {session.session_id} 的 Issue 已设定。")

            elif command == "/run":
                thread_id = self.assistant.get_current_thread_id()
                await self.assistant.run_step(thread_id)
                self._print_status(self.assistant.get_current_state())

            elif command == "/solve":
                state = self.assistant.get_current_state()
                ...
                try:
                    while True:
                        ...
                        try:
                            async for _step in self.assistant.run_auto_interactive(
                        ...
                        current_state = self.assistant.get_current_state()
                        ...
                        if last_drain > 0:
                            ...
                            self.assistant.reopen_after_terminal(thread_id)
                            continue
                        ...
                        if follow_up.startswith("!replan"):
                            ...
                            self.assistant.inject_message(thread_id, follow_up, replan=True)
                ...
                self.assistant.clear_runtime_sandbox()

                final_state = self.assistant.get_current_state()
                # 求解完成后，展示 diff 并询问是否推送
                if final_state.get("status") == "SUCCESS":
                    repo_path = final_state.get("repo_path")

                    diff = self.assistant.working_tree_diff_for_commit(repo_path)
                    if diff.strip():
                        print("\n" + "="*60)
                        print("📝 Agent 完成的修改内容：")
                        print("="*60)
                        ...
                        # 调用 Agent 生成提交计划
                        commit_plan = await self.assistant.build_commit_plan(
                            final_state,
                            diff,
                        )

                        report_markdown, pr_title, pr_body = self.assistant.build_fix_report(
                            final_state,
                            diff,
                            commit_plan,
                        )
                        session = self.assistant.current_session()
                        report_path = self.assistant.write_fix_report(
                            report_markdown,
                        ...
                        if commit_plan:
                            ...
                            if normalized_action in ("1", "commit", "push", "提交"):
                                add_result, commit_result, push_result = self.assistant.add_commit_push(
                                    commit_plan["files"],
                                    commit_plan["message"],
                                )
                                print(f"📦 添加结果:\n{add_result}")
                                ...
                                print("🚀 正在推送 PR 分支...")
                                _base_branch, output, _push_result, pr_url = self.assistant.create_pull_request_from_plan(
                                    repo_path=repo_path,
                                    files=commit_plan["files"],
                                    commit_message=commit_plan["message"],
                                    title=pr_title,
                                    body=report_markdown or pr_body,
                        ...
                        report_markdown, _, _ = self.assistant.build_fix_report(
                            final_state,
                            diff,
                            None,
                        )
                ...
                print("✅ 自动求解结束。")
                self._print_status(final_state or self.assistant.get_current_state())
    ...
    async def handle_input(self, text):
        ...
        response=await self.assistant.raw_chat(text)
        print(response)
...
def main():
    ...
    # ===== 初始化应用服务 =====
    print("🔧 初始化 IssueAssistantService...")
    assistant = IssueAssistantService.create_for_cli(os.getcwd())
    if assistant.loaded_skills:
        print(f"🎯 已加载 {len(assistant.loaded_skills)} 个 Skill: {', '.join(assistant.loaded_skills.keys())}")
    if assistant.registry is not None:
        print("   ✅ 已注册 {} 个工具，硬禁用: git_push".format(len(assistant.registry._tools)))
    if assistant.sandbox_manager:
